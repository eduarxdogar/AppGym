...
export class TrainerAiService {
  ...
  private buildPrompt(userPrompt: string, profile: UserProfile, mode: 'single' | 'weekly', daysToGenerate: number = 1, fatigueSummary?: string, historySummary?: string): string {
    ...
    if (level === 'principiante') {
        roleInstructions = "REGLA DE ROL (PRINCIPIANTE): Eres un coach protector, pero DEBES OBEDECER las directivas. Si el usuario pide SOBRECARGA (+2.5%), DEBES aplicar el aumento de peso ('pesokg') obligatoriamente en la plantilla base, sin importar si la fatiga está al 100%. Tienes PROHIBIDO cambiar los ejercicios de la plantilla original. Tu ÚNICA forma de proteger al novato es usar el campo 'coachNotes' para regañarlo y advertirle del riesgo de lesión por entrenar fatigado, pero la matemática de la sobrecarga debe aplicarse sí o sí.";
    } else {
