import { Component, inject, effect, NgZone, signal } from '@angular/core';
    ...
    <div class="min-h-screen w-full bg-background flex items-center justify-center p-4">
        <app-ui-card customClass="w-full max-w-md bg-black/40 backdrop-blur-md border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
            <div class="flex flex-col items-center text-center space-y-8 py-8 px-4">
                ...
                <!-- Loading State or Login Action -->
                <div class="w-full space-y-4 min-h-[100px] flex flex-col justify-center items-center">
                     <ng-container *ngIf="isCheckingAuth(); else loginButton">
                         <div class="flex flex-col items-center space-y-3">
                             <div class="w-8 h-8 border-4 border-green-500 border-t-transparent rounded-full animate-spin"></div>
                             <span class="text-sm text-gray-400">Verificando sesión...</span>
                         </div>
                     </ng-container>
...
export class LoginComponent {
  ...
  constructor() {
      // Se eliminó la redirección automática a petición del usuario.
      // Ahora el usuario SIEMPRE debe presionar el botón "Continuar con Google" 
      // para elegir su cuenta y avanzar, desactivando el "auto-login" silencioso.
  }
